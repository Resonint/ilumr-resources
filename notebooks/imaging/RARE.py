...
def soft_pulse(freq, phase, amp, width, bandwidth):
...
PARDEF = [
    ParDef('n_scans', int, 2, min=1, unit=''),
    ParDef('f', float, 1e6, unit='Hz'),
    ParDef('a_90', float, 0, min=0, max=1, unit=''),
    ParDef('t_90', float, 32e-6, unit='s'),
    ParDef('f_slice_offset', float, 0, unit='Hz'),
    ParDef('bw_90', float, 0, unit='Hz'),
    ParDef('a_180', float, 0, min=0, max=1, unit=''),
    ParDef('t_180', float, 32e-6, unit='s'),
    ParDef('t_inv', float, 1e-3, unit='s'),
    ParDef('t_dw', float, 1e-6, min=0.1e-6, max=80e-6, unit='s'),
    ParDef('n_samples', int, 320, min=2, unit=''),
    ParDef('t_read', float, 320e-6, unit='s'),
    ParDef('t_phase', float, 160e-6, unit='s'),
    ParDef('n_phase_1', int, 32, min=1, unit=''),
    ParDef('n_phase_2', int, 32, min=1, unit=''),
    ParDef('n_ETL', int, 0, min=0, unit=''),
    ParDef('t_end', float, 1, unit='s'),
    ParDef('g_slice', float_array, (0, 0, 0), min=(-1, -1, -1), max=(1, 1, 1), unit=''),
    ...
    ParDef('t_spoil', float, 1e-3, unit='s'),
    ParDef('g_spoil', float_array, (0, 0, 0), min=(-1, -1, -1), max=(1, 1, 1), unit=''),
    ParDef('t_grad_ramp', float, 100e-6, unit='s'),
    ParDef('n_grad_ramp', int, 10, min=2, unit=''),
...
def main(p: ParameterSet):
    ...
    if p.enable_dummy_run:
        ...
        if p.enable_IR:
            # inversion pulse
            yield seq.pulse_start(p.f, p_rf_acc, p.a_180)
        ...
        if enable_softpulse:
            ...
            yield (
                ...
                + soft_pulse(p.f+p.f_slice_offset, (p_90-p_rf_acc)%360, p.a_90, p.t_90, p.bw_90)
                + seq.wait(100e-9)
                + seq.pulse_update(p.f, 0, 0) # reset transmitter back to reference frame frequency
                + g_ramp_slice_to_zero
            )
            p_rf_acc += p_90_inc/2 + 360*p.f_slice_offset*(100e-9)
        else:
            yield (
        ...
        for i_echo in range(n_echos):
            ...
            if enable_softpulse:
                ...
                yield (
                    # refocusing pulse
                    seq.pulse_start(p.f, p_180, p.a_180)
                    + seq.wait(p.t_180)
                    + seq.pulse_end()
                )

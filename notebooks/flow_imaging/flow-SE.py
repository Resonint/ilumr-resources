...
PARDEF = [
    ...
    ParDef('n_samples', int, 128, min=2, unit=''),
    ParDef('t_echo', float, 2000e-6, unit='s'),
    ParDef('g_read', float_array, (0, 0, 0), min=(-1, -1, -1), max=(1, 1, 1), unit=''),
    ParDef('t_phase', float, 320e-6, unit='s'),
    ...
    ParDef('t_flow', float, 160e-6, unit='s'),
    ParDef('t_flow_spacing', float, 320e-6, unit='s'),
    ParDef('g_flow', float_array, (0, 0, 0), min=(-1, -1, -1), max=(1, 1, 1), unit=''),
    ...
    ParDef('n_grad_ramp', int, 10, min=1, unit=''),
...
def main(par: ParameterSet):
    ...
    # t_90/2 + t_flow + t_flow_spacing + t_flow + t_flow_180 + t_180/2 = t_echo/2
    t_flow_hold = par.t_flow - par.t_grad_ramp
    if t_flow_hold <= 0:
        raise Exception(f'Flow encode gradient time too short, minimum {par.t_grad_ramp}')
    t_flow_180 = (par.t_echo - par.t_90 - par.t_180)/2 - 2*t_flow_hold - 4*par.t_grad_ramp - par.t_flow_spacing
    if t_flow_180 <= 0:
        raise Exception('Echo time too short, or flow encoding/spacing too long.')
    ...
    for i_phase_1 in range(par.n_phase_1):
        ...
        for i_phase_2 in range(par.n_phase_2):
            ...
            for i_scan in range(par.n_scans):
                ...
                # wait
                yield seq.wait(par.t_flow_spacing)

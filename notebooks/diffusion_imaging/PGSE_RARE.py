...
PARDEF = [
    ...
    ParDef('n_phase_1', int, 32),
    ParDef('n_phase_2', int, 32),
    ParDef('n_ETL', int, 0),
...
def main(p: ParameterSet):
    ...
    t_unslice = max(p.t_90/2, 1e-6) # ensure positive and non-zero
    if enable_softpulse:
        ...
        g_unslice = -p.g_slice*0.5*(p.t_90+p.t_grad_ramp)/(t_unslice+p.t_grad_ramp)
...
#         #corr_phase_cycle = (np.array(phase_cycle_90) + phase_diff) % 360
    ...
    t_diff_echo_before = p.t_90/2 + p.t_grad_ramp + p.t_grad_ramp + t_unslice + p.t_grad_ramp + p.t_grad_ramp  + p.t_diff + p.t_grad_ramp + p.t_grad_ramp + p.t_180/2
    t_diff_echo_after = p.t_180/2 + p.t_grad_ramp + p.t_grad_ramp + p.t_diff + p.t_grad_ramp + p.t_grad_ramp + t_phase_read + p.t_grad_ramp + p.t_180/2
    t_diff_echo = 2*max(t_diff_echo_before, t_diff_echo_after) + 1e-6
    t_diff_echo_pad_before = t_diff_echo/2 - t_diff_echo_before
    t_diff_echo_pad_after = t_diff_echo/2 - t_diff_echo_after + t_echo/2
    log.debug('t_diff_echo: %.2e', t_diff_echo)
    
    # gradient duty cycle check TODO: implement duty cycle checks in driver
    t_cpmg = p.t_90/2 + t_echo/2 + p.n_phase_2*t_echo - p.t_180/2 + p.t_spoil
    t_rep = t_cpmg + p.t_end
    grad_total_area = (
        np.abs(p.g_slice)*(p.t_90+p.t_grad_ramp)
        + np.abs(p.g_read+g_unslice)*(t_phase_read+p.t_grad_ramp)
        + p.n_phase_2*2*(np.abs(p.g_phase_1)+np.abs(p.g_phase_2))*p.t_phase
        + p.n_phase_2*np.abs(p.g_slice)*p.t_180
    ...
    g_ramp_zero_to_unslice = gen_grad_ramp(g_ZERO, g_unslice, p.t_grad_ramp, p.n_grad_ramp)
    ...
    g_ramp_zero_to_diff = gen_grad_ramp(g_ZERO, p.g_diff, p.t_grad_ramp, p.n_grad_ramp)
    g_ramp_diff_to_zero = gen_grad_ramp(p.g_diff, g_ZERO, p.t_grad_ramp, p.n_grad_ramp)
    ...
    if p.enable_dummy_run:
        ...
        # slice phase correction
        yield seq.wait(p.t_grad_ramp)
        yield seq.wait(t_unslice)
        yield seq.wait(p.t_grad_ramp)
        yield seq.wait(t_diff_echo_pad_before)
        # diffusion gradient
        yield seq.wait(p.t_grad_ramp)
        yield seq.wait(p.t_diff)
        yield seq.wait(p.t_grad_ramp)
        ...
        yield seq.wait(p.t_diff)
        yield seq.wait(p.t_grad_ramp)
        yield seq.wait(t_diff_echo_pad_after)
    ...
    for i_scan in range(p.n_scans):
        ...
        for i_run in range(n_runs):
            ...
            # dummy diffusion gradient to improve stability
            yield g_ramp_zero_to_diff
            yield seq.wait(p.t_diff)
            yield g_ramp_diff_to_zero
            yield seq.wait(p.t_grad_ramp)
